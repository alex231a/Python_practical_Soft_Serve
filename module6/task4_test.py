...
class Group:
    ...
    # def __str__(self):
    #     return f"Group({self.title}, {self.students})"

    def __str__(self):
        student_info = [f"{student.full_name} ({student.avg_rank}): {student.courses}" for student in self.students]
        ...
        with open(students_file, 'r') as file:
            ...
            if isinstance(students_data, list):
                for student_data in students_data:
                    student = Student(student_data['full_name'], student_data['avg_rank'], student_data['courses'])
                    students.append(student)
            else:
                student = Student(students_data['full_name'], students_data['avg_rank'], students_data['courses'])
                students.append(student)
        return cls(title, students)

g2 = Group.create_group_from_file("jsons_task4/2020-01.json")
print(g2)